...
public class Deal {
	...
	public void choiceProduct() {
		...
		do {
			System.out.println("Хотите выбрать товар?" + "\n 1. Да" + "\n 2. Нет"); // предлагаем пользователю выбрать
																					// товар,
			int ch = scanner.nextInt();
			if (ch == 2) { // если нет, то формируем корзину выбранных товаров, выводим чек
				printBasket(bayer.getProducts());
				break;
			}
			System.out.println("Выберите товар из списка: "); // если да,предлагаем список товаров
			...
			int productPosition = scanner.nextInt(); // позиция товара в списке, выбранная пользователем
			Product selected = seller.getProducts()[productPosition - 1]; // выбранный пользователем продукт извлекаемиз
																			// списка
			...
			while (ch > selected.getQuantity()) { // проверяем введенное количество
				System.out.println("Вы ввели количество больше " + selected.getQuantity() + ". Повторите ввод: ");
				ch = scanner.nextInt();
			}

			boolean isAddProductBasket = true;
			for (int i = 0; i < bayer.getProducts().length; i++) { // проверяем, есть ли уже товар с таким наименованием
			...
			if (isAddProductBasket) { // если такого товара нет в корзине, то
				bayer.setProducts(Arrays.copyOf(bayer.getProducts(), bayer.getProducts().length + 1)); // пересоздаем
																										// корзину с
																										// увеличением
																										// на 1 размера
				selected.setQuantity(selected.getQuantity() - ch);
				selected.setSelectedCount(ch);// записали в объект введенное количство товара
				bayer.getProducts()[bayer.getProducts().length - 1] = seller.getProducts()[productPosition - 1]; // добавляем
																													// в
																													// корзину
																													// выбранный
																													// продукт
